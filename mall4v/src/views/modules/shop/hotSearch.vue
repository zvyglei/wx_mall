  ...
  <div class="mod-hotSearcch">
    <avue-crud ref="crud" :page="page" :data="dataList" :table-loading="dataListLoading" :option="tableOption"
      ...
      <template slot="menuLeft">
        <el-button v-if="isAuth('admin:hotSearch:save')" type="primary" size="small" icon="el-icon-plus"
                   @click="addOrUpdateHandle()">新增</el-button>
        <el-button type="danger" size="small" @click.stop="deleteHandle" :disabled="dataListSelections.length <= 0">批量删除
        </el-button>
      </template>
      <template slot-scope="scope" slot="status">
        <el-tag v-if="scope.row.status === 0" size="small" type="danger">未启用</el-tag>
        <el-tag v-else size="small">启用</el-tag>
      </template>
      <template slot-scope="scope" slot="menu">
        <el-button v-if="isAuth('admin:hotSearch:update')" type="primary" size="small" icon="el-icon-edit"
                   @click="addOrUpdateHandle(scope.row.hotSearchId)">修改</el-button>
        <el-button v-if="isAuth('admin:hotSearch:delete')" type="danger" icon="el-icon-delete" size="small"
