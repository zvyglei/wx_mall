    ...
    <avue-crud ref="crud"
      ...
      <template slot="menuLeft">
        <el-button type="primary"
                   icon="el-icon-plus"
                   size="small"
                   v-if="isAuth('prod:prod:flash-sale:save')"
                   @click.stop="addOrUpdateHandle()">新增</el-button>

        <el-button type="danger"
                   @click="deleteHandle()"
                   size="small"
                   v-if="isAuth('prod:prod:flash-sale:delete')"
      ...
      <template slot-scope="scope"
                slot="flashSaleTime">
        <span>{{ scope.row.flashSaleTime }} 秒</span>
      </template>
      ...
      <template slot-scope="scope"
        ...
        <el-button type="primary"
                   icon="el-icon-edit"
                   size="small"
                   v-if="isAuth('prod:prod:flash-sale:update')"
...
<script>
import { tableOption } from '@/crud/prod/prodFlashSaleList'
export default {
  data () {
    return {
      dataForm: {
      ...
      permission: {
        delBtn: this.isAuth('prod:prod:flash-sale:delete')
      },
      dataList: [],
      page: {
        total: 0, // 总页数
  ...
  methods: {
    ...
    getDataList (page, params, done) {
      ...
      this.$http({
        url: this.$http.adornUrl('/prod/prod/flash-sale/page'),
        method: 'get',
