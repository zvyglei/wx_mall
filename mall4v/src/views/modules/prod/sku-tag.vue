  ...
  <div class="mod-prod-sku-tag">
    <el-form-item label="商品规格">
      ...
      <div v-for="(tag, tagIndex) in skuTags" :key="tagIndex">
        ...
        <br />
        <el-tag v-for="(tagItem, tagItemIndex) in tag.tagItems" :key="tagItem.valueId" closable
                :disable-transitions="false" @close="handleTagClose(tagIndex, tagItemIndex)">
          {{tagItem.propValue || '默认'}}
        </el-tag>
        <el-input class="input-new-tag" v-if="tagItemInputs[tagIndex] && tagItemInputs[tagIndex].visible"
                  v-model="tagItemInputs[tagIndex].value" :ref="`saveTagInput${tagIndex}`" size="small"
                  @keyup.enter.native="handleInputConfirm(tagIndex)" @blur="handleInputConfirm(tagIndex)">
        </el-input>
    ...
    <el-row>
      <el-col :span="6">
        <el-form-item label="规格名" v-show="isShowTagInput">
          <el-select v-model="addTagInput.propName" filterable allow-create default-first-option placeholder="请选择"
                     @change="handleTagClick">
            <el-option v-for="item in unUseTags" :key="item.propId" :label="item.propName" :value="item.propName">
            </el-option>
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :span="6">
        <el-form-item label="规格值" v-show="isShowTagInput">
          <el-select v-model="addTagInput.selectValues" multiple filterable allow-create default-first-option
                     placeholder="请选择">
            <el-option v-for="item in dbTagValues" :key="item.valueId" :label="item.propValue" :value="item.propValue">
            </el-option>
          </el-select>
        </el-form-item>
      </el-col>
    </el-row>
    <el-form-item v-show="isShowTagInput">
      <el-button size="mini" type="primary" @click="addTag()">确定</el-button>
      <el-button size="mini" @click="hideTagInput()">取消</el-button>
    </el-form-item>
  </div>
</template>
