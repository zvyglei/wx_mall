						...
						uni.showModal({
							...
							success: res => {
								if (res.confirm) {
									...
									var url = ''
									// #ifdef H5 || MP-WEIXIN
									if (uni.getStorageSync('appType') == AppType.MP || uni.getStorageSync('appType') ==
										AppType.MINI) {
						...
						uni.showModal({
							...
							success: res => {
								if (res.confirm) {
									...
									// #ifdef H5
									uni.navigateTo({
										url: uni.getStorageSync('appType') == AppType.MP ? '/pages/login/login' :
