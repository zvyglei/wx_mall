  ...
  <view class="main">
    ...
    <radio-group class="radio-group">
      <block v-for="(item, index) in addressList" :key="index">
        <view class="address">
          <view class="personal u-flex" @tap="selAddrToOrder(item)">
            <view style="width: 90%;">
							<view class="info-tit">
								<text class="name">{{item.receiver}}</text>
								<text class="tel">{{item.mobile}}</text>
								<!-- <image src="/static/images/icon/revise.png" @tap.stop="toEditAddress" :data-addrid=""></image> -->
							</view>
							<view class="addr">
								<text class="addr-get"> {{item.province}}{{item.city}}{{item.area}}{{item.addr}}</text>
							</view>
            </view>
						<u-icon class="u-flex-1" size="32" name="edit-pen" @tap.stop="toEditAddress(item.addrId)"></u-icon>
          </view>
	...
	<view class="btn" v-if="!isSelect">
		<u-button type="primary" shape="circle" @click="onAddAddr">
			<text>新建收货地址</text>
		</u-button>
	</view>
...
export default {
  ...
  methods: {
    ...
    toEditAddress: function (addrId) {
      // uni.navigateTo({
      //   url: '/pages/address/add-or-update?type=add&addrId=' + addrId
      // });
			uni.navigateTo({
				url: `/pages/address/add-or-update?type=update&addrId=${addrId}`
			});
    },
