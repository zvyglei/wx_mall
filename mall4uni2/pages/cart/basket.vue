<template>
	...
	<view class="container">
			...
			<block v-for="(item, scIndex) in shopCartItemDiscounts" :key="scIndex">
				<view class="prod-block">
					<view class="discount-tips" v-if="item.chooseDiscountItemDto">
						...
						<text
							class="text-list">{{wxs.parseDiscountMsg(item.chooseDiscountItemDto.discountRule,item.chooseDiscountItemDto.needAmount,item.chooseDiscountItemDto.discount)}}</text>
					</view>
					<block v-for="(prod, index) in item.shopCartItems" :key="index">
						<view class="item">
							<view class="btn">
								<label>
									<checkbox @tap="onSelectedItem" :data-scindex="scIndex" :data-index="index"
										:value="toString(prod.prodId)" :checked="prod.checked" color="#105c3e"></checkbox>
								</label>
							</view>
							<view class="prodinfo">
								<view class="pic">
		...
		<view class="cart-footer" v-if="shopCartItemDiscounts.length>0">
			<view class="btn all">
				<label @tap="onSelAll">
					<checkbox :checked="allChecked" color="#f7d731;"></checkbox>全选
				</label>
			</view>
			...
			<view class="btn total">
				<view class="finally">
					...
					<view class="price">
						<!-- <text class="symbol">￥</text> -->
						<image src="/static/jindou.png" style="width: 32rpx; height: 32rpx; margin-bottom: -6rpx;"></image>
						<text class="big-num">{{finalMoney}}</text>
	...
	export default {
		...
		methods: {
			...
			toFirmOrder: function() {
				...
				uni.navigateTo({
					url: '/pages/order/submit?orderEntry=0'
				});
			},
