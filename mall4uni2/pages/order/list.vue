				...
				<view class="prod-item">
					<view class="order-num">
						...
						<view class="order-state">
							<text
								:class="'order-sts  ' + (item.status==1?'red':'') + '  ' + ((item.status==5||item.status==6)?'gray':'')">{{item.status==1?'待支付':(item.status==2?'待发货':(item.status==3?'待收货':(item.status==5?'已完成':'已取消')))}}</text>

							<view class="clear-btn" v-if="item.status==5 || item.status==6">
								<image src="/static/images/icon/clear-his.png" class="clear-list-btn" @tap="delOrderList"
								...
								<view class="item-cont" @tap="toOrderDetailPage" :data-ordernum="item.orderNumber">
									...
									<view class="prod-info">
										...
										<view class="price-nums">
											<image src="/static/jindou.png" style="width: 32rpx; height: 32rpx; margin-bottom: -6rpx;">
											</image>
					...
					<view class="prod-foot">
						<view class="btn">
							<text v-if="item.status==1" class="button" @tap="onCancelOrder" :data-ordernum="item.orderNumber"
								hover-class="none">取消订单</text>
							<!-- <text class="button warn" :data-ordernum="item.orderNumber" hover-class="none">再次购买</text> -->
							<text v-if="item.status==1" class="button warn" @tap="normalPay" :data-ordernum="item.orderNumber"
								hover-class="none">付款</text>
							<text v-if="item.status==3 || item.status==5" class="button" @tap="toDeliveryPage"
	...
	export default {
		...
		methods: {
				...
				var params = {
					...
					callBack: res => {
						...
						uni.requestPayment({
							...
							success: function() {
								uni.navigateTo({
									url: '/pages/pay-result/pay-result?sts=1&orderNumbers=' + e.currentTarget.dataset
										.ordernum
								});
							},
			...
			toOrderDetailPage: function(e) {
				uni.navigateTo({
					url: '/pages/order/detail?orderNum=' + e.currentTarget.dataset.ordernum
				});
			},
