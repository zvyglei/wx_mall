<template>
	<view>
		...
		<u-modal :show="payPasswordShow" title="请输入支付密码" :showConfirmButton="flase" :showCancelButton="true"
			@cancel="payPasswordShow = false">
			<view class="slot-content">
				<u-code-input v-model="payPassWord" :maxlength="6" :dot="true" @finish="toPay"></u-code-input>
				<view class="u-m-t-10">
					<u--text @tap="toForgetPay" :text="'忘记支付密码？'" :type="'primary'"></u--text>
				</view>
			</view>
		</u-modal>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toForgetPay() {
				uni.navigateTo({
					url: "/pages/user/forget-pay-password"
				})
			},
			/**
			 * 提交订单
			...
			submitOrder: function() {
				...
				var params = {
					...
					method: "POST",
					data: {
						payPassWord: encrypt(this.payPassWord),
						orderShopParam: [{
				...
				var params = {
					...
					callBack: function(res) {
						...
						uni.requestPayment({
							...
							fail: err => {
								uni.navigateTo({
									url: '/pages/pay-result/pay-result?sts=0&orderNumbers=' + orderNumbers +
										"&orderType=" + this.orderType
								});
							}
						});
			...
			toAddrListPage: function() {
				console.log(1213213312)
				uni.navigateTo({
					url: '/pages/address/delivery-address?order=0'
				});
			},
