        ...
        <view class="btn">
          ...
          <button v-if="status==3" class="button warn" @tap="onConfirmReceive" :data-ordernum="orderNumber" hover-class="none">确认收货</button>
				...
				<view class="msg-item">
					...
					<!-- <view class="item">
						...
						<view class="item-txt price">
							...
							<text class="">{{wxs.parsePrice(reduceAmount)[0]}}</text>
						</view>
					</view> -->
			...
			<view class="btn" v-if="!isSelect">
				<u-button type="info" shape="circle" v-if="status==5||status==6" @tap="delOrderList">
					<text>删除订单</text>
				</u-button>
			</view>
			<!-- 底部栏 -->
				...
				var params = {
					...
					callBack: res => {
						this.orderNumber = orderNum,
							...
							this.userAddrDto = res.userAddrDto,
							this.remarks = res.remarks,
							this.orderItemDtos = res.orderItemDtos,
							this.createTime = res.createTime,
							this.status = res.status,
							this.productTotalAmount = res.orderItemDtos[0].productTotalAmount,
							this.transfee = res.transfee,
							this.reduceAmount = res.reduceAmount,
